  ...
  return (
    <div className="modal-overlay">
      <div className="modal-content-lg">
        ...
        {/* Two-column body — left: location/signature, right: composition/notes */}
        <div className="modal-body p-5 grid grid-cols-2 gap-5">
          ...
          <div className="space-y-3.5">
            ...
            <div className="bg-slate-950/60 border border-violet-500/20 rounded-lg p-3 space-y-2">
              ...
              {signature !== '' && (
                topMatches.length === 0 ? (
                  <div className="text-[11px] text-slate-600 font-mono">No match found.</div>
                ) : (
                  <div className="space-y-1.5">
                    {topMatches.map((m) => (
                      <button
                        ...
                        className="w-full flex items-center justify-between gap-2 bg-slate-900 hover:bg-slate-800/80 border border-slate-800 hover:border-violet-500/30 rounded-lg px-2.5 py-1.5 transition-all cursor-pointer text-left"
                      >
                        <span className="flex items-center gap-1.5">
                          <span className={`text-[11px] font-mono font-bold ${TIER_COLORS[m.entry.tier]}`}>{m.entry.ore}</span>
                          <span className="text-[10px] font-mono text-slate-500">×{m.nodeCount}</span>
                        </span>
                  ...
                  <div key={idx} className="flex items-center gap-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        onWheel={(e) => e.currentTarget.blur()}
                        placeholder="0"
                        className="w-14 bg-slate-950 border border-slate-800 text-xs text-amber-400 font-mono text-center rounded-lg px-1.5 py-1.5 focus:outline-none focus:border-sky-500"
                      />
                      <span className="text-xs text-slate-500 font-mono">%</span>
                    </div>
                    {ores.length > 1 && (
                      <button onClick={() => removeOre(idx)} className="text-slate-700 hover:text-red-400 transition-colors cursor-pointer flex-shrink-0">
        ...
        <div className="modal-footer flex justify-end gap-2 px-5 py-3.5 border-t border-slate-800">
